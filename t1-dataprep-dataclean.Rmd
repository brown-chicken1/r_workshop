...
#### Preparing Intersection Point Map
* For the site survey analysis, a plot of road intersection density will be used to measure the road density in a particular area.
  ...
  ![hi](images/1DataPrep6.png)
  
  + In the resulting window, just select the newly dissolved layer and click "Run".
  
  ![hi](images/1DataPrep7.png)
  

  + The final layer that is created, would be soon utilised to create an intersection.
  
  + We click on that layer, go to the PRocessing Toolbox on the right hand side and search for "Line Intersections". Open the tool and select both the input and intersect layer as the recently created layer from the previous step. 
  
   ![hi](images/1DataPrep10.png)
   
   ![hi](images/1DataPrep11.png)
  
  + We need to first remove any duplicate intersections to clean the layer data a bit further. We will go to the Processing Toolbox at the bottom right hand corner of the page, and then search for "Delete Duplicate Geometries". When the window opens, we use the layer that we had just created in the previous step as the Input layer, and then run.
  ...
  + Save the newly created intersection layer by right clicking on the point layer and selecting Save Features As, and in the resultant box that comes up, ensure that the GeoPackage file that the base map layer was selected, is selected under the file name and the layer name is set to "Intersections", and the map CRS is set to EPSG:23845.
...
* Then, click on Run, and a temporary  buffer layer should be generated.
...
6c. Now, click on the newly saved GeoPackage layer containing the point data. Go to Vector > Geoprocessing Tool > Buffer. In the resultant window, make a buffer of 100m x 100m by changing the distance to 100m and the units to meters, and make
