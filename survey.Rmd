...
![](sitesurvimg/Sea ports location.png)
...
The road network naturally gets more dense around the urban areas of Balikpapan and Samarinda. There are a few major throroughfares highlighted in blue on the map, indicating primary roads and motorways. They primarily connect Balikpapan with Samarinda, and from Samarinda, there is a road further inland tnto Kalimantan out of the study area.

Similarly the secondary road network is also relatively well developed, especially along the coastal areas in the north of the study area. 

However, some areas remain relatively untouched, especially the west and southwest of the study area where the terrain is more hilly and they are far from the urban population centres.
...
Using count points in polygon to create a choropleth map does not accurately tell us where the forest fires are concentrated at. So we decided to plot a heatmap for the forest fires to get a better understanding as to the precise locations and concentration of the forest fires in our study area.

```{r}
knitr::include_graphics("img/heatmapFF.png")
```

The heatmap reveals that the top two districts with the most forest fires does not have forest fires evently spread across the entire district. Rather, most of the forest fires are concentrated in certain areas of the districts. 
...
For digital elevation, the DEM map gave a range of -77m under sea level to 289m above sea level. We would want to avoid areas with low elevation and especially if it is close to water bodies (lakes and rivers) so as to avoid flash floods. 

```{r}
knitr::include_graphics("img/E1.png")
```

Firstly, we made any area below sea level as blue to signify that these areas are either filled with water or are extremely likely to. Areas with less than 10m above sea level are considered dangerously low-lying and susceptible to flash floods. Hence we considered anything below 10m as dangerous (red-orange) and anything above 15 would be considered relatively safe in the near future. Any area with an elevation of above 50m and 100m would be considered "mountainous areas", hence we have these areas in light and dark shades of yellow.
...
```{r}
knitr::include_graphics("img/E3.png")
``` 
...
```{r}
knitr::include_graphics("img/E4.png")
```
