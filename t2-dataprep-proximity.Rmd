...
## 1. Objective: 
The objective of this section is to create raster layers and further using the raster layers to create proximity maps (distance) as well as to normalise the values of the proximity map in order to accurate derive selection criteria analysis from it.
...
>DIY: Using the steps you had learned, Clip the proximity map to Final_shape and save the newly computed proximity map into GeoPackage format with a resolution of 100m x 100m. Call the layer prox_road.
...
## 4.2 Clipping proximity map to study area 
...
For the input layer of the Clip Raster by Mask Layer window, put in prox_road. For the mask layer, put in final_area.

For the Source CRS and Target CRS, ensure that both of them are EPSG : 23845 DGN95 / Indonesia TM-3 zone 54.1. 

For the section 'Assign a specific nodata value to output bands [optional]', change it to 99999999. Your screen should look like this :

```{r}
knitr::include_graphics("img/clip1.png")
```

When you are done, click on 'Run'. A new layer called Clipped by Mask Layer would appear. Save it as a geopackage and call it clippedprox_road.


## 4.3 Working with Raster Calculator function of QGIS
...
+ At the Layers panel of Raster Calculator dialog window, double-click on clippedprox_road, it will appear in the expression. Select the divide key “/” and input the maximum value of the layer you are using. In this case for roads is “13263.8613281250”
...
## 5.0 Working with Proximity function of QGIS

>* DIY: Using the steps you learnt in this section, replicate the steps to create raster layers and normalised proximity map layers for: 
+ settlements
+ forest
+ airport
+ seaport
